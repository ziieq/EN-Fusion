...
import torchmetrics
from .dataset_fusion import MalDataset
...
def train_single_side(model, train_set, test_set, side, output_dim=2, epochs=20, device='cuda:0'):
    ...
    for epoch in range(epochs):
        for sid, labels, features in tqdm.tqdm(dataloader):
            # 正向传播
...
def train_MSL(model, train_set, test_set, side, output_dim=2, epochs=20, device='cuda:0'):
    print('*** Start MSL training：')
    # 定义损失函数、优化器、DataLoader
    criterion = nn.CrossEntropyLoss()
    ...
    stage_list = ['end', 'net', 'fusion']
    for stage in stage_list:
        print(rf'*** Start MSL stage：{stage} side')
        # 开始训练
        for epoch in range(epochs):
            for sid, labels, features in tqdm.tqdm(dataloader):
                # 正向传播
                y_pre = model.forward(features, stage)
                # 反向传播
                optimizer.zero_grad()
                loss = criterion(y_pre, labels)
                loss.backward()
...
def predict_set(model, test_set, side):
    ...
    for sid, labels, features in tqdm.tqdm(dataloader):
        ...
        y_score_all += y_pre.tolist()
        y_pre = torch.argmax(y_pre, dim=1)
        y_pred_all += y_pre.tolist()
...
def run_training(model, data_dir_path, side, output_dim, dst_model_dir_path, epoch, f_len, s_len):
    device = 'cuda:0'
    torch.cuda.init()

    # 实例化Dataset类
    is_binary = True if output_dim == 2 else False
    train_set = MalDataset(rf'{data_dir_path}/id_train.pkl', rf'{data_dir_path}/x_train.pkl', rf'{data_dir_path}/y_train.pkl', bi=is_binary, data_type=side, device=device, f_len=f_len, s_len=s_len)
    test_set = MalDataset(rf'{data_dir_path}/id_test.pkl', rf'{data_dir_path}/x_test.pkl', rf'{data_dir_path}/y_test.pkl', bi=is_binary, data_type=side, device=device, f_len=f_len, s_len=s_len)

    # 开始训练
    if model is None:
        model = ENFusion(output_dim).to(device)
    if side == 'fusion':
        train_MSL(model, train_set, test_set, side, output_dim=output_dim, epochs=epoch, device=device)
    else:
